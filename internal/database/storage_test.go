...
var testDB *sql.DB
var testUserStore BaseUserStore
var testFollowStore BaseFollowStore
var testPostStore BasePostStore
var testCommentStore BaseCommentStore
var testLikeStore BaseLikeStore
var testReplyStore BaseReplyStore
var testTagStore BaseTagStore

func NewPostgresTestDB() *sql.DB {
	godotenv.Load("../../.env")
...
func TestUserStore_CreateUser(t *testing.T) {
	...
	err := testUserStore.CreateUser(ctx, user)
	assert.NoError(t, err)

	existUser, err := testUserStore.GetUserByID(ctx, user.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		_ = testUserStore.DeleteUser(ctx, existUser.ID)
	})
}
...
func TestUserStore_UpdateUser(t *testing.T) {
	...
	err = testUserStore.UpdateUser(ctx, user)
	assert.NoError(t, err)

	updatedUser, err := testUserStore.GetUserByID(ctx, user.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		_ = testUserStore.DeleteUser(ctx, updatedUser.ID)
	})
}
...
func TestUserStore_DeleteUser(t *testing.T) {
	...
	deletedUser, err := testUserStore.GetUserByID(ctx, user.ID)
...
func TestUserStore_GetUserByUsername(t *testing.T) {
	...
	existUser, err := testUserStore.GetUserByUsername(ctx, "test")
	assert.NoError(t, err)
	assert.NotNil(t, existUser)
...
func TestUserStore_GetUsersByUsername(t *testing.T) {
	...
	err := testUserStore.CreateUser(ctx, userJohn)
	assert.NoError(t, err)

	err = testUserStore.CreateUser(ctx, userJohnny)
	assert.NoError(t, err)

	err = testUserStore.CreateUser(ctx, userTest)
	assert.NoError(t, err)

	t.Run("Get users by username success", func(t *testing.T) {
		users, err := testUserStore.GetUsersByUsername(ctx, "john")
		assert.NoError(t, err)
	...
	t.Run("Get users by username fail", func(t *testing.T) {
		users, err := testUserStore.GetUsersByUsername(ctx, "z")
		assert.ErrorIs(t, err, sql.ErrNoRows)
		assert.Nil(t, users)
	})

	t.Cleanup(func() {
		_ = testUserStore.DeleteUser(ctx, userJohn.ID)
		_ = testUserStore.DeleteUser(ctx, userJohnny.ID)
		_ = testUserStore.DeleteUser(ctx, userTest.ID)
...
func TestUserStore_GetUserByEmail(t *testing.T) {
	...
	existUser, err := testUserStore.GetUserByEmail(ctx, "test@test.com")
	assert.NoError(t, err)
	assert.NotNil(t, existUser)
...
func TestFollowStore_FollowUser(t *testing.T) {
	...
	err := testUserStore.CreateUser(ctx, user1)
	assert.NoError(t, err)

	err = testUserStore.CreateUser(ctx, user2)
	assert.NoError(t, err)
...
func TestFollowStore_GetFollowerByUserID(t *testing.T) {
	...
	followings, err := testFollowStore.GetFollowerByUserID(ctx, user2.ID)
	assert.NoError(t, err)
...
func TestFollowStore_GetFollowinsByUserID(t *testing.T) {
	...
	followings, err := testFollowStore.GetFollowingByUserID(ctx, user1.ID)
	assert.NoError(t, err)
	assert.NotEmpty(t, followings)
	assert.Equal(t, 1, len(followings))
	assert.Equal(t, user1.ID, followings[0].UserID)
...
func TestPostStore_CreatePost(t *testing.T) {
	...
	err = testPostStore.CreatePost(ctx, post)
	assert.NoError(t, err)

	existPost, err := testPostStore.GetPostByID(ctx, post.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		_ = testPostStore.DeletePost(ctx, post.ID)
		_ = testUserStore.DeleteUser(ctx, user.ID)
	})
...
func TestPostStore_UpdatePost(t *testing.T) {
	...
	err = testPostStore.UpdatePost(ctx, post)
	assert.NoError(t, err)

	updatedPost, err := testPostStore.GetPostByID(ctx, post.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		_ = testPostStore.DeletePost(ctx, updatedPost.ID)
		_ = testUserStore.DeleteUser(ctx, user.ID)
	})
...
func TestPostStore_DeletePost(t *testing.T) {
	...
	err = testPostStore.DeletePost(ctx, post.ID)
	assert.NoError(t, err)

	deletedPost, err := testPostStore.GetPostByID(ctx, post.ID)
	assert.ErrorIs(t, err, sql.ErrNoRows)
...
func TestPostStore_GetPostsByUserIDByLimit(t *testing.T) {
	...
	fivePosts, err := testPostStore.GetPostsByUserID(ctx, user.ID, pagination, search)
	assert.NoError(t, err)
	assert.Equal(t, 5, len(fivePosts))

	pagination.Limit = 10

	allPosts, err := testPostStore.GetPostsByUserID(ctx, user.ID, pagination, search)
	assert.NoError(t, err)
...
func TestPostStore_GetPostDetailsByID(t *testing.T) {
	...
	existPost, err := testPostStore.GetPostDetailsByID(ctx, post.ID, user.ID)
	assert.NoError(t, err)
	assert.NotNil(t, existPost)
...
func TestPostStore_HasAccessToPost(t *testing.T) {
	...
	hasAccess, err := testPostStore.HasAccessToPost(ctx, user2.ID, post.ID)
	assert.NoError(t, err)
...
func TestCommentStore_CreateComment(t *testing.T) {
	...
	err = testCommentStore.CreateComment(ctx, comment)
	assert.NoError(t, err)

	existComment, err := testCommentStore.GetCommentByID(ctx, comment.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		_ = testCommentStore.DeleteComment(ctx, comment.ID)
		_ = testUserStore.DeleteUser(ctx, user.ID)
	})
}
...
func TestCommentStore_UpdateComment(t *testing.T) {
	...
	err = testCommentStore.UpdateComment(ctx, comment)
	assert.NoError(t, err)

	updatedComment, err := testCommentStore.GetCommentByID(ctx, comment.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		...
		_ = testCommentStore.DeleteComment(ctx, updatedComment.ID)
...
func TestCommentStore_DeleteComment(t *testing.T) {
	...
	err = testCommentStore.DeleteComment(ctx, comment.ID)
	assert.NoError(t, err)

	deletedComment, err := testCommentStore.GetCommentByID(ctx, comment.ID)
	assert.ErrorIs(t, err, sql.ErrNoRows)
...
func TestLikeStore_LikePost(t *testing.T) {
	...
	err = testLikeStore.LikePost(ctx, like)
	assert.NoError(t, err)
	isLiked, err := testLikeStore.IsPostLiked(ctx, post.ID, user.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		...
		_ = testPostStore.DeletePost(ctx, post.ID)
		_ = testLikeStore.UnlikePost(ctx, post.ID, user.ID)
		_ = testUserStore.DeleteUser(ctx, user.ID)
	})
}
...
func TestLikeStore_UnlikePost(t *testing.T) {
	...
	err = testLikeStore.UnlikePost(ctx, post.ID, user.ID)
	assert.NoError(t, err)
...
func TestLikeStore_LikeComment(t *testing.T) {
	...
	err = testLikeStore.LikeComment(ctx, like)
	assert.NoError(t, err)
	isLiked, err := testLikeStore.IsCommentLiked(ctx, comment.ID, user.ID)
	assert.NoError(t, err)
	assert.Equal(t, true, isLiked)
	...
	t.Cleanup(func() {
		...
		_ = testPostStore.DeletePost(ctx, post.ID)
		_ = testCommentStore.DeleteComment(ctx, comment.ID)
		_ = testLikeStore.UnlikeComment(ctx, post.ID, user.ID)
		_ = testUserStore.DeleteUser(ctx, user.ID)
	})
}
...
func TestLikeStore_UnlikeComment(t *testing.T) {
	...
	err = testLikeStore.UnlikeComment(ctx, comment.ID, user.ID)
	assert.NoError(t, err)
	isLiked, err = testLikeStore.IsCommentLiked(ctx, post.ID, user.ID)
	assert.NoError(t, err)
	assert.Equal(t, false, isLiked)
...
func TestReplyStore_CreateReply(t *testing.T) {
	...
	err = testReplyStore.CreateCommentReply(ctx, reply)

	assert.NoError(t, err)

	existReply, err := testReplyStore.GetReplyByID(ctx, reply.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		...
		_ = testPostStore.DeletePost(ctx, post.ID)
		_ = testCommentStore.DeleteComment(ctx, comment.ID)
		_ = testReplyStore.DeleteReply(ctx, post.ID)
		_ = testLikeStore.UnlikeComment(ctx, post.ID, user.ID)
		_ = testUserStore.DeleteUser(ctx, user.ID)
	})
}
...
func TestReplyStore_CreateNestedReply(t *testing.T) {
	...
	err = testReplyStore.CreateNestedReply(ctx, nestedReply)
	assert.NoError(t, err)

	existNestedReply, err := testReplyStore.GetReplyByID(ctx, nestedReply.ID)
	assert.NoError(t, err)
	...
	t.Cleanup(func() {
		...
		_ = testPostStore.DeletePost(ctx, post.ID)
		_ = testCommentStore.DeleteComment(ctx, comment.ID)
		_ = testReplyStore.DeleteReply(ctx, reply.ID)
		_ = testReplyStore.DeleteReply(ctx, nestedReply.ID)
		_ = testLikeStore.UnlikeComment(ctx, post.ID, user.ID)
		_ = testUserStore.DeleteUser(ctx, user.ID)
	})
}
...
func TestReplyStore_UpdateReply(t *testing.T) {
	...
	err = testReplyStore.UpdateReply(ctx, reply)
	assert.NoError(t, err)

	updatedReply, err := testReplyStore.GetReplyByID(ctx, reply.ID)
	assert.NoError(t, err)
...
func TestReplyStore_DeleteReply(t *testing.T) {
	...
	err = testReplyStore.DeleteReply(ctx, existReply.ID)
	assert.NoError(t, err)

	deletedReply, err := testReplyStore.GetReplyByID(ctx, existReply.ID)
	assert.ErrorIs(t, err, sql.ErrNoRows)
...
func TestReplyStore_LikeReply(t *testing.T) {
	...
	err = testLikeStore.LikeReply(ctx, replyLike)
	assert.NoError(t, err)

	isLiked, err := testLikeStore.IsReplyLiked(ctx, reply.ID, user.ID)
	assert.NoError(t, err)
	assert.Equal(t, true, isLiked)
...
func TestReplyStore_UnlikeReply(t *testing.T) {
	...
	err = testLikeStore.UnlikeReply(ctx, reply.ID, user.ID)
	assert.NoError(t, err)
...
func TestReplyStore_GetRepliesByCommentID(t *testing.T) {
	...
	commentReplies, err := testReplyStore.GetRepliesByCommentID(ctx, comment.ID, user.ID)
	assert.NoError(t, err)
...
func TestReplyStore_GetRepliesByParentID(t *testing.T) {
	...
	replies, err := testReplyStore.GetRepliesByParentID(ctx, nestedReply.ParentID, user.ID)
	assert.NoError(t, err)
...
func TestTagStore_AddPostTags(t *testing.T) {
	...
	err = testPostStore.CreatePost(ctx, post1)
	assert.NoError(t, err)

	post2 := createTestPost(t, "test", user.ID, "public")
	err = testPostStore.CreatePost(ctx, post2)
	assert.NoError(t, err)

	tags := createTestTags("test1", "test2")
	tagStrs := []string{"test1", "test2"}

	err = testTagStore.CreateMultipleTags(ctx, tags)
	assert.NoError(t, err)

	err = testTagStore.CreatePostTag(ctx, tagStrs, post1.ID)
	assert.NoError(t, err)

	err = testTagStore.CreatePostTag(ctx, []string{"test1"}, post2.ID)
	assert.NoError(t, err)

	pagination := createTestPagination(t)
	posts, err := testPostStore.GetPostsByTag(ctx, pagination, "test1", user.ID)
	assert.NoError(t, err)
	assert.Equal(t, 2, len(posts))

	posts, err = testPostStore.GetPostsByTag(ctx, pagination, "test2", user.ID)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(posts))

	t.Run("Create post tag fail", func(t *testing.T) {
		err = testTagStore.CreatePostTag(ctx, nil, post2.ID)
		assert.Error(t, err)
	})

	t.Cleanup(func() {
		_ = testUserStore.DeleteUser(ctx, user.ID)
		_ = testPostStore.DeletePost(ctx, post1.ID)
		_ = testPostStore.DeletePost(ctx, post2.ID)
	})
}

func TestMain(m *testing.M) {
	db := NewPostgresTestDB()
	testDB = db
	testUserStore = NewUserStore(db)
	testFollowStore = NewFollowStore(db)
	testPostStore = NewPostStore(db)
	testCommentStore = NewCommentStore(db)
	testLikeStore = NewLikeStore(db)
	testReplyStore = NewReplyStore(db)
	testTagStore = NewTagStore(db)
	cleanupAllTables()
	code := m.Run()
	testDB.Close()
	os.Exit(code)
